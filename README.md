...
- **口语录音模式**
  新增 `/speaking.html`，可手动输入题目或文段，保存为练习卡片，再在同一个题目下反复录制多个 answer attempts。录音默认限制为 2 分钟，适合 IELTS Part 2，也可以切换为 30 秒、1 分钟、3 分钟或不限时；每条录音都可以单独播放、转写、分析和删除。该模式不调用火山 TTS。
